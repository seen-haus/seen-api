...
class CollectableOutputTransformer extends BaseTransformer {
    transform(collectable) {
        return {
            ...
            nft_token_id: collectable.nft_token_id && collectable.nft_token_id.indexOf(',') > 0 ? collectable.nft_token_id.replace(/\s/g,'').split(',').map(item => Number(item)) : collectable.nft_token_id,
